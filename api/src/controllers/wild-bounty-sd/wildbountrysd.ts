...
import wildbountysdfunctions from "../../functions/wild-bounty-sd/bountyfunctions"
// IMPORT LINHAS
import linhaperdabounty from "../../jsons/wild-bounty-sd/linhaperdabouty"
import linhabonusbikini from "../../jsons/bikini-paradise/linhabonusbikini"
import notcashbounty from "../../jsons/wild-bounty-sd/notcashbouty"
import linhaganhobounty from "../../jsons/wild-bounty-sd/linhaganhobouty"
   ...
   async spin(req: Request, res: Response) {
      ...
      try {
         ...
         if (resultadospin.result === "perda") {
            ...
            await wildbountysdfunctions.attsaldobyatk(token, newbalance)
            await wildbountysdfunctions.atualizardebitado(token, bet)
            await wildbountysdfunctions.atualizarapostado(token, bet)
            const perdajson = await linhaperdabounty.linhaperda()
         ...
         if (resultadospin.result === "ganho") {
            try {
               ...
               let linhaGanhoAnterior = await wildbountysdfunctions.obterLinhaGanho(userId)
               ...
               const ganhojson = await linhaganhobounty.linhaganho(numeroAleatorio)
               ...
               await wildbountysdfunctions.atualizardebitado(token, bet)
               await wildbountysdfunctions.atualizarapostado(token, bet)
               await wildbountysdfunctions.atualizarganho(token, valorganho)
               ...
               let json = {
                  dt: {
                     si: {
                        ...
                        prbr: ganhojson[steps].prbr,
                        orl: ganhojson[steps].orl,
                        rns: ganhojson[steps].rns,
                        sc: ganhojson[steps].sc,
                        gm: ganhojson[steps].gm,
                        ngm: ganhojson[steps].ngm,
                        gsb: ganhojson[steps].gsb,
                        gs: ganhojson[steps].gs,
                        ngs: ganhojson[steps].ngs,
                        wgs: ganhojson[steps].wgs,
                        ssaw: valorganho,
               ...
               await wildbountysdfunctions.atualizarLinhaGanho(userId, numeroAleatorio)
               ...
               if (getSteps(token, gamename) >= Object.keys(ganhojson).length) {
                  ...
                  await wildbountysdfunctions.atualizarLinhaGanho(userId, null) // Resetar a linha de ganho quando os steps são resetados
               }
