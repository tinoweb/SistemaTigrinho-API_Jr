...
export default {
  ...
  async savejsonspin(id: number, json: any) {
    const res = await promisePool.query<ResultSetHeader>("UPDATE chickyrunjson SET JSON = ? WHERE id = ?", [json, id]);
    return res[0];
  },

  async getjsonchicky(id: number) {
    const res = await promisePool.query<RowDataPacket[]>("SELECT * FROM chickyrunjson WHERE id=?", [id]);
    return res[0];
  },

  async createjsonchicky(id: number) {
    const res = await promisePool.query<ResultSetHeader>("INSERT INTO chickyrunjson (id) VALUES (?)", [id]);
    return res[0];
  },
