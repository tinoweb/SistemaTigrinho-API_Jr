...
export default {
   ...
   async savejsonspin(id: number, json: any) {
      const res = await promisePool.query<ResultSetHeader>(
         "UPDATE ultimatestrikerjson SET JSON = ? WHERE id = ?",
         [json, id],
      )
      return res[0]
   },
   async getjsonstriker(id: number) {
      const res = await promisePool.query<RowDataPacket[]>(
         "SELECT * FROM ultimatestrikerjson WHERE id=?",
         [id],
      )
      return res[0]
   },
   async createjsonstriker(id: number) {
      const res = await promisePool.query<ResultSetHeader>(
         "INSERT INTO ultimatestrikerjson (id) VALUES (?)",
         [id],
      )
