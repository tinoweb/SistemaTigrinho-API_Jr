...
export default {
  ...
  async savejsonspin(id: number, json: any) {
    const res = await promisePool.query<ResultSetHeader>("UPDATE majestictsjson SET JSON = ? WHERE id = ?", [json, id]);
    return res[0];
  },

  async getjsonmajestic(id: number) {
    const res = await promisePool.query<RowDataPacket[]>("SELECT * FROM majestictsjson WHERE id=?", [id]);
    return res[0];
  },

  async createjsonmajestic(id: number) {
    const res = await promisePool.query<ResultSetHeader>("INSERT INTO majestictsjson (id) VALUES (?)", [id]);
    return res[0];
  },
