...
export default {
  ...
  async savejsonspin(id: number, json: any) {
    const res = await promisePool.query<ResultSetHeader>("UPDATE wildapejson SET JSON = ? WHERE id = ?", [json, id]);
    return res[0];
  },

  async getjsonwildape(id: number) {
    const res = await promisePool.query<RowDataPacket[]>("SELECT * FROM wildapejson WHERE id=?", [id]);
    return res[0];
  },

  async createjsonwildape(id: number) {
    const res = await promisePool.query<ResultSetHeader>("INSERT INTO wildapejson (id) VALUES (?)", [id]);
    return res[0];
  },
