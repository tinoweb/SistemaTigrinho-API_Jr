...
export default {
   ...
   async savejsonspin(id: number, json: any) {
      const res = await promisePool.query<ResultSetHeader>("UPDATE piggygoldjson SET JSON = ? WHERE id = ?", [json, id])
      return res[0]
   },
   async getjsonpiggy(id: number) {
      const res = await promisePool.query<RowDataPacket[]>("SELECT * FROM piggygoldjson WHERE id=?", [id])
      return res[0]
   },
   async createjsonpiggy(id: number) {
      const res = await promisePool.query<ResultSetHeader>("INSERT INTO piggygoldjson (id) VALUES (?)", [id])
