...
export default {
  ...
  async savejsonspin(id: number, json: any) {
    const res = await promisePool.query<ResultSetHeader>("UPDATE butterflyblossomplayerjson SET JSON = ? WHERE id = ?", [json, id]);
    return res[0];
  },

  async getjsonbutterfly(id: number) {
    const res = await promisePool.query<RowDataPacket[]>("SELECT * FROM butterflyblossomplayerjson WHERE id=?", [id]);
    return res[0];
  },

  async createjsonbutterfly(id: number) {
    const res = await promisePool.query<ResultSetHeader>("INSERT INTO butterflyblossomplayerjson (id) VALUES (?)", [id]);
    return res[0];
  },
