...
export default {
   ...
   async savejsonspin(id: number, json: any) {
      const res = await promisePool.query<ResultSetHeader>(
         "UPDATE cashmaniaplayerjson SET JSON = ? WHERE id = ?",
         [json, id],
      )
      return res[0]
   },
   async getjsoncash(id: number) {
      const res = await promisePool.query<RowDataPacket[]>(
         "SELECT * FROM cashmaniaplayerjson WHERE id=?",
         [id],
      )
      return res[0]
   },
   ...
   async attawcall(id: number, valor: number) {
      const res = await promisePool.query<ResultSetHeader>(
         `UPDATE calls SET aw = '${valor}' WHERE id='${id}'`,
      )

      return res[0]
   },
   async createjsoncash(id: number) {
      const res = await promisePool.query<ResultSetHeader>(
         "INSERT INTO cashmaniaplayerjson (id) VALUES (?)",
         [id],
      )
